...
void getCropped(Mat &img, Mat &dest) {
    ...
    Mat z;
    z = img(Rect2d(Point(l, t), Point(r, b)));
...
void getResults(Mat &hand, int *dest) {
    ...
    for (int i = 0; i < 128; i++) {
        ...
        if (!top_in && *(getPixel(hand, i, 30)) > 10) {
            top_in = true;
        } else if (top_in && *(getPixel(hand, i, 30)) == 0) {
            top_in = false;
            top_count++;
        }
    }
...
int getNumber(int *a) {
    if(a[6]<100){return 0;}
    if(a[6]>3000000){return 1;}
    if (a[0] == 1 && a[1] == 1, a[2] == 1) { return 2; }
    if (a[0] == 1 && a[1] == 1 && a[2] == 2 && a[3] == 0 && a[4] == 1 && a[6] / 100000 > 7) { return 8; }
    if (a[0] == 1 && a[1] == 1 && a[2] == 2 && a[3] == 0 && a[4] == 1 && a[6] / 100000 <= 7) { return 7; }
    if (a[0] == 1 && a[1] == 1 && a[2] == 2) { return 6; }
    if (a[0] == 2 && a[1] == 0 && a[2] == 2 && a[3] == 0 && a[4] == 0) { return 6; }
    if (a[0] == 2 && a[1] == 1 && a[2] == 2) { return 4; }
    if (a[0] == 2 && a[1] == 1 && a[2] == 1) { return 4; }
    if (a[0] == 2 && a[1] == 0 && a[2] == 2) { return 3; }
    if (a[0] == 3 && a[1] == 0) { return 5; }
    if (a[0] == 2 && a[1] == 1 && a[2] == 2 && a[3] == 0 && a[4] == 2 && a[5] == 0) { return 9; }
    if (a[0] == 1 && a[1] == 1 && a[2] == 2 && a[6] / 100000 < 10) { return 8; }
    return -1;
}
...
int main() {
    ...
    Mat frame;
    cap >> frame;
    Rect2d region(Point(frame.cols / 3, frame.rows / 4), Point(2 * frame.cols / 3, 3 * frame.rows / 4));
    Mat bg;
    getBG(bg, cap, region);
    imshow("bg", bg);
    waitKey(0);
    ...
    while (cap.isOpened()) {
        ...
        if (n.cols > 0) {
            ...
            putText(frame, str1.str(), Point(frame.cols / 2, frame.rows / 2), FONT_HERSHEY_SIMPLEX, 4,
                    Scalar(255, 255, 255));
        }
        imshow("frame", frame);
        if (nn.cols > 0) {
